    ...
    describe "Checkpoint - You're all winners" do
        ...
        describe "#legal_positions" do
            it "should return an array of positions" do
                ...
                expect(legal.length).to eq(24)
                expect(legal[0].length).to eq(2)
            end

            it "should have values that are valid and empty" do
                grid = board.instance_variable_get(:@grid)
                grid[0][0] = :X

                legal = board.legal_positions
                expect(legal.any? { |pos| pos[0] == 0 && pos[1] == 0 }).to eq(false)
                expect(legal.any? { |pos| pos[0] == 0 && pos[1] == 1 }).to eq(true)
                expect(legal.any? { |pos| pos[0] == 5 && pos[1] == 5 }).to eq(false)
            end

            it "should use #valid?" do
                expect(board).to receive(:valid?).at_least(:once)
                board.legal_positions
            end

            it "should use #empty?" do
                expect(board).to receive(:empty?).at_least(:once)
                board.legal_positions
            end
        end
    end
